...
Cookies use `SameSite=Lax`, `HttpOnly`, `Secure` in production, and narrow paths (`/api` for access and `/api/v1/auth` for refresh). CORS remains an explicit allow-list and CSRF defenses are completed in the hardening phase.
...
- Cross-site state-changing requests still require the planned CSRF boundary; `SameSite` is defense-in-depth, not the sole control.
