...
Eventory signs a compact versioned payload with HMAC-SHA256. The payload includes an opaque public ticket code, a one-way event-session binding, a per-ticket nonce, a key version, and a signature. It never includes attendee PII or raw database IDs. The active key version is configured through `QR_KEY_VERSION`; unsupported versions are rejected.
...
- Key rotation requires a compatibility window or an explicit re-issuance strategy before changing the active version in a production deployment.
